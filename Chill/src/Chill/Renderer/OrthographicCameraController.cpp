#include "chpch.h"
#include "Chill/Renderer/OrthographicCameraController.h"

#include "Chill/Core/Input.h"
#include "Chill/Core/KeyCodes.h"
...
namespace Chill {
    ...
    void OrthographicCameraController::Update(Timestep ts)
    {
        ...
        else if (Input::IsKeyPressed(CH_KEY_D))
            m_CameraPosition.x += m_CameraTranslationSpeed * ts;

        if (Input::IsKeyPressed(CH_KEY_W))
            m_CameraPosition.y += m_CameraTranslationSpeed * ts;
        else if (Input::IsKeyPressed(CH_KEY_S))
            m_CameraPosition.y -= m_CameraTranslationSpeed * ts;

        if (m_Rotation)
        {
            if (Input::IsKeyPressed(CH_KEY_Q))
                m_CameraRotation += m_CameraRotationSpeed * ts;
            if (Input::IsKeyPressed(CH_KEY_E))
                m_CameraRotation -= m_CameraRotationSpeed * ts;
